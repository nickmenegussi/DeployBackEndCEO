...
exports.viewAllLectures = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(`SELECT * FROM Lecture`);
...
exports.createLecture = async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    const [existingLecture] = await connection.query(
    ...
    const [result] = await connection.query(
      `INSERT INTO Lecture (nameLecture, dateLecture, timeLecture, description, link_url, video_url)
       VALUES (?, ?, ?, ?, ?, ?)`,
...
// ========================== UPDATE LINK ==========================
exports.updateLectureLinkUrl = async (req, res) => {
  let connection;
  const { idLecture } = req.params;
